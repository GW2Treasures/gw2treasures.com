...
export const ItemsContainerContent: Job = {
  run: async (ids: number[] | undefined) => {
    if(!Array.isArray(ids)) {
      const containerIds = await db.item.findMany({ where: { OR: [{ type: 'Container' }, { type: 'Consumable', subtype: 'Immediate' }], contains: { none: {}}, containsCurrency: { none: {}}}, select: { id: true }});
