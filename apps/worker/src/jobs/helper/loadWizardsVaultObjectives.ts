...
export async function loadWizardsVaultObjectives(ids: number[]) {
  ...
  const [de, en, es, fr] = await Promise.all([
    fetchApi(`/v2/wizardsvault/objectives?ids=${ids.join(',')}`, { language: 'de' }),
    fetchApi(`/v2/wizardsvault/objectives?ids=${ids.join(',')}`, { language: 'en' }),
    fetchApi(`/v2/wizardsvault/objectives?ids=${ids.join(',')}`, { language: 'es' }),
    fetchApi(`/v2/wizardsvault/objectives?ids=${ids.join(',')}`, { language: 'fr' }),
  ]);
