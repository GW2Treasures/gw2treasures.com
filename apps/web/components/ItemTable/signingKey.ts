...
class SigningKeyClient {
  ...
  async getKey() {
    ...
    console.log(styleText('bgCyan', '*** GENERATED SIGNING KEY: ' + Buffer.from(JSON.stringify(exported)).toString('base64') + ' ***'));
    console.log(styleText('cyan', 'Set this key as SIGNING_KEY environment variable (.env.local) or signatures will not be valid after restarts.'));
