...
export const CurrenciesUpdate: Job = {
  run: async (ids: number[] | Record<string, never>) => {
    ...
    for(const { existing, de, en, es, fr } of currencies) {
      ...
      const revision_de = changed_de ? await db.revision.create({ data: { data: JSON.stringify(de), language: 'de', buildId, type: 'Update', entity: 'Currency', description: 'Updated in API', schema }}) : existing.current_de;
      const revision_en = changed_en ? await db.revision.create({ data: { data: JSON.stringify(en), language: 'en', buildId, type: 'Update', entity: 'Currency', description: 'Updated in API', schema }}) : existing.current_en;
      const revision_es = changed_es ? await db.revision.create({ data: { data: JSON.stringify(es), language: 'es', buildId, type: 'Update', entity: 'Currency', description: 'Updated in API', schema }}) : existing.current_es;
      const revision_fr = changed_fr ? await db.revision.create({ data: { data: JSON.stringify(fr), language: 'fr', buildId, type: 'Update', entity: 'Currency', description: 'Updated in API', schema }}) : existing.current_fr;
