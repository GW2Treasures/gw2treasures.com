...
}, ['four-winds-achievements'], { revalidate: 60 * 60 });
...
export default async function FourWindsAchievementsPage({ params }: PageProps) {
  ...
  await pageView('festival/four-winds/achievements');
  ...
  return (
    <PageLayout>
      ...
      <AchievementTable achievements={achievements} language={language}>
        {(table, ColumnSelect) => (
          <>
            <Description actions={ColumnSelect}><Trans id="festival.four-winds.achievements.description"/></Description>
            {table}
          </>
        )}
      </AchievementTable>
...
export const generateMetadata = createMetadata(async ({ params }) => {
  ...
  return {
    ...
    description: t('festival.four-winds.achievements.description'),
    url: 'festival/four-winds/achievements'
  };
});
