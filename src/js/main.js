(function(window, document, $) {
	$(document).ready(function() {
		...
		if(ga) { ga(removeAnalyticsParameter); }
	...
	var tooltips = {
		init: function() {
			tooltips.getBounds();
			$(document)
				.on( 'mouseenter', 'a[data-item-id]', function() { tooltips.showTooltip( $(this) ) } )
				.on( 'mouseleave', 'a[data-item-id]', tooltips.hideTooltip );
			tooltips.tooltip = $('<div/>').attr('id', 'tooltip');
			$('body').prepend(tooltips.tooltip);
			$('body').on('mousemove', function(e) {
				var x = Math.min( Math.max( e.pageX, tooltips.bounds.x ), tooltips.bounds.width - tooltips.tooltip.width() );
				var y = Math.min( Math.max( e.pageY, tooltips.bounds.y ), tooltips.bounds.height - tooltips.tooltip.height() );
				tooltips.tooltip.css('transform', 'translate(' + x + 'px,' + y + 'px)');
			});
		...
		getBounds: function( ) {
			tooltips.bounds = { x: 0, y: 0, width: $(document).width(), height: $(document).height() };
		},
		...
		showTooltip: function( obj ) {
			...
			if( cache.has( 'tooltip:' + lang + ':' + id )) {
				cache.get( 'tooltip:' + lang + ':' + id, function( t ) {
					tooltips.tooltip.html( t );
				});
			} else {
				tooltips.tooltip.html( '' );
				tooltips.loadTooltip( id, $this, lang );
			}
		},

		loadTooltip: function( id, obj, lang ) {
			$.get( '//' + lang + '.' + window.gw2treasures.domain + '/item/' + id + '/tooltip', function(data) {
				cache.put( 'tooltip:' + lang + ':' + id, data, 360 );
