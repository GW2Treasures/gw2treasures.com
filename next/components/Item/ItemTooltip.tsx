...
function getData(item: Item): ItemData {
  return JSON.parse(item.data_en);
}
...
export const ItemTooltip: FC<ItemTooltipProps> = ({ item }) => {
  ...
  return (<>
    {item.type === 'Weapon' && `Strength: ${data.details?.min_power} – ${data.details?.max_power}`}
    {item.type === 'Armor' && `Defense: ${data.details?.defense}`}
  </>);
};
