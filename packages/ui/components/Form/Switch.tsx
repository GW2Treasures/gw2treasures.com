...
export type SwitchControlProps = {
  ...
  | { type?: 'button', href?: never, clickAction: () => void, name?: string, value?: string }
  | { type: 'link', href: string, name?: never, value?: never }
  | { type: 'radio', href?: never, name: string, value: string }
);
...
Switch.Control = ({ children, active, type = 'button', href, clickAction, name, value, icon, tip }) => {
  const Element = type === 'link' ? Link : type === 'radio' ? 'label' : 'button';

  const element = (
    <Element href={href!} onClick={clickAction} className={active && type !== 'radio' ? styles.controlActive : styles.control} name={type === 'button' ? name : undefined} value={type === 'button' ? value : undefined}>
