...
export async function loadColors(ids: number[]) {
  ...
  const [de, en, es, fr] = await Promise.all([
    fetchApi(`/v2/colors?ids=${ids.join(',')}`, { language: 'de' }),
    fetchApi(`/v2/colors?ids=${ids.join(',')}`, { language: 'en' }),
    fetchApi(`/v2/colors?ids=${ids.join(',')}`, { language: 'es' }),
    fetchApi(`/v2/colors?ids=${ids.join(',')}`, { language: 'fr' }),
  ]);
